...
## R-28 · כניסה למסך ריצה חדשה יוצרת טיוטה מיד — ✅ **נסגר (2026-09-21, ADR-0042)**: הטיוטה נוצרת בעריכה הראשונה; יציאה ממסך שלא נגעו בו לא משאירה רשומה (בדיקה: `compactUx.test.tsx`). (הוסף 2026-07-31)
...
## 2026-09-20 — מגבלות הרחבת פרויקט הריצה
- ~~טרם בוצעה בדיקת מובייל חזותית 390×844~~ — **בוצעה בערב 2026-09-20 ב-Chrome אמיתי (Claude Code)**; 12 ליקויים תוקנו. ראיות: `docs/ai-runs/2026-09-20/.../assets/mobile-qa-390x844/`.
- מנוע ההמלצות מקומי ומבוסס כללים; אינו סוכן LLM ואינו מופעל ברקע כאשר האפליקציה סגורה.
- התאריכים המדויקים של המרוצים לא אומתו. התאמה לפי תאריך מופעלת רק לאחר שהמשתמש מזין תאריך.
- הנתונים החדשים מקומיים כמו יומני האימונים הקיימים. אימות משתמש/סנכרון Phase 1 בענן עדיין לא נסגר; אין להכריז על cloud readiness.

## R-43 · `notFound()` ב-loader של השרת הופך רענון ל-HTTP 404 — ✅ **נסגר (2026-09-21)** (הוסף 2026-09-20 ערב)
**תרחיש:** ישויות חיות ב-localStorage; בשרת הן תמיד חסרות. loader שזורק `notFound()` בשרת מחזיר **404 אמיתי** לכל ניווט ישיר/רענון של כתובת קיימת — גם כשהרשומה קיימת במכשיר. שונה מ-R-26 (זריקה ב-render): כאן זו תשובת HTTP.
**נצפה:** `/running/{id}` ו-`/running/{id}/edit` — כולל **כתובת הטיוטה שהטופס מציב** (`replaceState`) — החזירו 404 ברענון, גם ב-live (`curl` → 404).
**תוקן (PR #1):** שני מסלולי הריצה: ה-loader מחזיר `{id}` כש-`typeof window === "undefined"`, והרכיב מיישם ADR-0039 (`useHydrated` → `notFound()` בלקוח בלבד). 404 אמיתי לכתובת שגויה נשמר (בדיקת `runningRouteLoaders`).
**נותר:** ~~`treadmills.$id`, `locations.$id`, `exercises.$id`~~ — **2026-09-21:** `treadmills.$id` תוקן באותו דפוס (ADR-0039) עם בדיקת רגרסיה ב-`compactUx.test.tsx`; `locations`/`exercises` כבר החזירו 200 (אומת ב-`curl`). אין מסלול נוסף שזורק `notFound()` ב-loader.

## R-44 · הקלדה לפני סיום hydration אינה נקלטת — 🟢 Low (הוסף 2026-09-20 ערב)
**תרחיש:** ב-SSR הטופס מוצג לפני ש-React משתלט; הקלדה באותו חלון זמן (מאות ms ב-dev) אובדת. נצפה ב-QA אוטומטי בלבד, ב-dev עם cold cache. ב-production ה-hydration מהיר; לא נצפה אובדן במסך אמיתי. לא טופל — התנהגות React/SSR כללית.

## R-45 · פיצול נתונים בין האתר לאפליקציית Android — 🟡 Medium (הוסף 2026-09-21)
**תרחיש:** ה-APK (origin `https://localhost`) והאתר (`fitlog-workout.lovable.app`) מחזיקים localStorage נפרד. דיווח בשניהם לסירוגין יוצר שני יומנים שאינם מסונכרנים עד שסנכרון Phase 1 בענן יאומת (T-05).
**מיטיגציה:** `docs/ai/android.md` — מעבר חד-פעמי Export→Restore ומקור אמת אחד (האפליקציה). ייבוא חוזר מוסיף חדשים בלבד ולא דורס (אומת באמולטור: 0 חדשות / 31 זהות / 67 התנגשויות לא נדרסו).
...
**מיטיגציה:** ה-workflow שומר את `~/.config/.android/debug.keystore` (הנתיב ב-runner של Ubuntu) ב-`actions/cache` (מפתח קבוע) כך שה-APK הבא מתקין מעל הקודם כל עוד ה-cache חי (7 ימים ללא שימוש → נמחק); תמיד לייצא גיבוי לפני שדרוג; להתקין תמיד מאותו מקור (artifact של `main`). release keystore קבוע דורש אישור של אריאל (יצירת credential) — לא נוצר.
...
## R-48 · שני מסלולי הזנת משך מתקיימים במקביל — 🟢 Low (הוסף 2026-09-24)
**תרחיש:** טופס Suunto משתמש ב-`DurationTextField` (שדה טקסט אחד, חוזה ADR-0044), בעוד `RunForm` משתמש ב-`DurationField` (שני שדות מספר, דקות/שניות). שני המסלולים שומרים שניות ואינם ניתנים לקריאה דו-משמעית, אך המשתמש נתקל בשתי צורות הזנה שונות לאותו סוג נתון.
**למה לא אוחד:** שני שדות מספריים מתויגים הם חד-משמעיים מבנית; המרה שלהם היא churn ללא פגם מוכח (§19).
**טריגר לטיפול:** אם אריאל ידווח על בלבול בין שני המסכים, או אם יתווסף שדה משך שלישי — לאחד ל-`DurationTextField`.

## R-49 · `owner_id` מרוכז אך עדיין קבוע — 🟡 Medium (הוסף 2026-09-24)
**תרחיש:** ADR-0047 ריכז את הבעלות ב-`currentOwnerId()`, אך הערך עדיין `"single-user"` קבוע. כל עוד אין session מאומת (T-05), רשומות שייווצרו במכשיר אינן נושאות זהות אמיתית; חיבור עתידי ל-`auth.uid()` יצטרך להחליט מה עושים עם רשומות שנוצרו תחת `"single-user"`.
**מיטיגציה בפועל:** הערך המאוחסן לא השתנה ולכן שום רשומה לא זזה; `visibleToCurrentOwner()` מתייחס לרשומה ללא `owner_id` כשייכת לבעלים המקומי, כך ששום דבר לא נעלם. ההחלטה נדחית במודע עד שיהיה session מאומת.
**מה שלא נעשה בכוונה:** לא הומצא user id ל-Ariel ולא הוטמע כקבוע קסם (§15).

## R-50 · מטרות מגע במסכים חדשים נבדקות רק ב-QA, לא בבדיקה אוטומטית — 🟢 Low (הוסף 2026-09-24)
**תרחיש:** שני ליקויי 44px (`32x32` בפקדי שורת הפק״ל, `22x36` ב-breadcrumb של בנק התרגילים) התגלו רק ב-QA באמולטור ובדפדפן אמיתי, אחרי שכל 512 הבדיקות עברו. jsdom אינו מודד פריסה ולכן אינו יכול לתפוס זאת.
**מיטיגציה:** סקריפט ה-QA (`assets/web-qa.txt`, `assets/android-recheck.txt`) מדווח כל אלמנט מתחת ל-44px בכל מסך; יש להריץ אותו לכל מסך חדש.
