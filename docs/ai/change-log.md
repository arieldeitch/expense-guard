...
## 2026-09-24 · פידבק שימוש אמיתי: קלט Suunto/ריצה · פק״לים לפי כמות · בנק תרגילי בית · מוכנות ריבוי משתמשים (Claude Code)

- **ADR-0044 (קלט):** parser מרכזי אחד ב-`src/lib/runs/calc.ts` — `parseDurationInput` (`ss` · `mm:ss` · `hh:mm:ss`; מספר חשוף = **שניות**; כל חלק אחרי הראשון 0..59; אחרת `null`), `parsePaceMSS` מאציל אליו, `formatDurationInput`, `describeDuration`, `parseDecimal` (גם פסיק עשרוני). `src/lib/forms/useNumericText.ts` שומר טקסט גולמי בזמן הקלדה כך ש-`7.` שורד עד `7.15`. `src/components/inputs/NumericField.tsx` — `DecimalField` ו-`DurationTextField` עם הד חי (`role="status"`, `aria-describedby`); כל 15 שדות המספר ב-`SuuntoForm` הומרו. **שינוי התנהגות מכוון:** `parsePaceMSS("30")` היה 1800 והוא כעת 30.
- **ADR-0045 (פק״לים):** `src/lib/home/pakalim.ts` — שתי שגרות קבועות (`tpl_pakal_morning`, `tpl_pakal_evening`), seeding idempotent שאינו דורס template קיים, `openPakalSession` מחזיר את דיווח היום (מפתח יום Asia/Jerusalem) במקום ליצור חדש, `pakalLines` pure (סט נוצר בעריכה הראשונה), `setPakalQuantity`, `pakalTotal`. מסך `/home/pakal/$slot`: כמות עם ± בגודל 44px, שינוי סדר, הסרה, הוספה מהבנק, סיכום, שינוי שם השגרה. `PakalCard` במסך הבית. `startQuickEntry` כבר אינו יורש `name_he` של תרגיל — שם ברירת המחדל הוא "דיווח מהיר". שורת היסטוריה: שם השגרה ככותרת, התרגילים כהקשר.
- **ADR-0046 (בנק תרגילים):** `src/lib/exercises/homeBank.ts` — משקל גוף · דאמבלים · חבל; חמש קבוצות שריר לדאמבלים לפי **קבוצת קודים** (`["back","lats","traps"]`), `firstSlug` ממוין ראשון ומסומן "הנפוץ ביותר"; `bodyRegionOfExercise`. `HomeExerciseAdder` — בורר דו-שלבי עם breadcrumb וחיפוש. `MuscleBadge` (צללית מטרה) ו-`ExerciseMovementIllustration` (זוגות START→END, 10 תנועות) עם `decorative` כדי לא לזהם שם נגיש. ב-seed נוספו **רק** `Wide Push Ups` ו-`Dumbbell Floor Press`; ארבעה תרגילים שהוצעו כבר היו קיימים ולכן הקבוצות מצביעות עליהם (קנוניזציה, לא שכפול). `MuscleMap.tsx` לא שונה.
- **ADR-0047 (בעלות):** `src/lib/identity/owner.ts` — `LOCAL_OWNER_ID = "single-user"` (**הערך המאוחסן לא השתנה, ולכן אין migration**), `SYSTEM_OWNER_ID`, `currentOwnerId()` כנקודה היחידה שבה יישב `auth.uid()`, `withOwner()` כתפר בדיקות, `visibleToCurrentOwner()` (רשומה ישנה ללא `owner_id` = הבעלים המקומי). שמונה מודולי אחסון מייצאים `CURRENT_OWNER_ID` מהמקור הזה. **אין UX של משתמש שני, אין family management, אין parent/child, אין נתונים עבור אדם אחר.**
- **בדיקות:** `duration-contract.test.ts` (9), `pakalim.test.ts` (10), `home-bank.test.ts` (7), `owner.test.ts` (7), `pakalimUx.test.tsx` (8, `test:router:pakalim`). **423 יחידה / 31 קבצים + 89 מסכים / 14 קבצים = 512.**
- **שערים:** frozen install ✅ · typecheck ✅ · build ✅ · `git diff --check` ✅ · סודות ✅ · lint: **0 שגיאות ו-0 אזהרות בכל הקבצים ששונו**; הריפו 367 בעיות, כולן בקבצים שלא נגענו בהם (baseline R-33/R-17) · `routeTree.gen.ts` — ההפרש הוא בדיוק המסלול החדש `/home/pakal/$slot` (R-30).
- **QA:** Web 390×844 RTL ב-Chrome אמיתי · Android אמולטור Pixel 7 API 36 על ה-APK המותקן (מקלדת רכה אמיתית, Back חומרה, force-stop, offline, זהות build). שני ליקויי מטרת-מגע שנמצאו ב-QA תוקנו (`ae74039`, `b851b55`). **קבלה במכשיר פיזי אינה מסומנת PASS.** ראיות ב-`docs/ai-runs/2026-09-24/fitness_app_recovery-20260924-pakalim-suunto-home-ux/assets/`.
- **נתונים:** אין שינוי סכימה, RLS, policy, מיגרציה, תלויות או lockfile. לא נמחק `localStorage` אמיתי; `fitlog:qa-marker` שרד וקבוצת המפתחות `fitlog:*` זהה. לא הועלו נתוני Ariel לענן.
- **Android:** גרסה 1.2.0 (versionCode 10200). build flow ללא שינוי.
