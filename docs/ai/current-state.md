...
- **Git (מעודכן 2026-07-25, אחרי סנכרון `main`):** **`main` = `origin/main` = `f33d00a`** — עודכן ב-**fast-forward** מ-`feat/domain-alignment-and-restore` (ללא merge commit) ונדחף ללא force. **`origin/main` הוא מקור האמת** לגרסה הנוכחית. אימות מלא הורץ על `main`: typecheck exit 0 · 216 בדיקות · eslint 0 errors / 8 baseline warnings · build ×2 · `routeTree.gen.ts` ללא שינוי. `eca9163` נשמר בהיסטוריה. ה-feature branch **לא נמחק**. **לא בוצע deploy.**
