...
## הרחבה תואמת לאחור — 2026-09-20
ב־`fitlog:runs:v1` נוספו אוספים אופציונליים `races`, `trainingWeeks`, `coachSettings`. השבוע מכיל המלצה מקורית, בחירה אישית, דילוג וגרסאות קודמות. `RunSession` מכיל קישורים אופציונליים `training_plan_item_id` ו־`race_id`; הביצוע נשאר ביומן הקיים. `HomeSession.training_partner` הוא הקשר טקסטואלי בלבד, ללא פרופיל או נתוני אימון של ילד. הגיבוי הקיים כולל את האוספים; אין מיגרציית DB ואין מפתח אחסון נוסף.
