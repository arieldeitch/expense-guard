...
## 🎯 הפעולה הבאה (נקבע ב-Recovery Audit, 2026-07-30)

### 0. 🔴 חוסם — פעולה ידנית של המשתמש (לא ניתנת לביצוע ע"י סוכן)
**ייצוא גיבוי בפועל.** לפתוח `/backup` בגרסת `main` המאומתת (`daba93c`), לייצא, ולשמור את הקובץ **מחוץ לאחסון הדפדפן** (מכשיר נפרד או תיקייה מגובה). עד אז R-22 פתוח בפועל — **יכולת גיבוי אינה גיבוי**.

### 1. הפעולה הבאה היחידה לפיתוח — **תיקון R-24** (רק אחרי הגיבוי)
- **מטרה עסקית:** Restore מקומי לא ייבא רשומות יתומות ולא ייצור נתונים שבורים במכשיר.
- **Scope:** `src/lib/backup/repo.ts` בלבד — `REFERENCE_RULES` + לולאת האימות, ובדיקות נלוות.
- **Out of scope:** ה-import pipeline של הענן, ה-UI, פורמט ה-Export, schema 1.0.0, כל מודול אחסון אחר.
- **קבצים צפויים:** `src/lib/backup/repo.ts` · `src/lib/backup/__tests__/*` · `docs/ai/{change-log,risks,decisions}.md`.
- **Acceptance criteria:** הכלל משתמש ב-`home_session_id` · נוסף כלל `home.sets → home.entries` על `entry_id` · `parents.size === 0` אינו מדלג בשקט · **קובץ גיבוי תקין ממשיך לעבור** (רגרסיה) · קובץ עם הפניה שבורה מייצר `dangling_reference`.
- **בדיקות:** `bun run test:unit` (backup 14 → צפוי לגדול) + `bun run test` + typecheck + eslint + build.
- **סיכון:** **שינוי התנהגות מכוון** — גיבויים עם הפניות שבורות שהתקבלו עד היום **ייפסלו**. זו ההתנהגות הנכונה, אך היא מצדיקה ADR מפורש ואסטרטגיית תאימות לאחור.
- **Rollback:** ענף נפרד, commit יחיד הפיך; `main` אינו נוגע עד merge מאושר.
- **החלטה אנושית נדרשת לפני התחלה?** **כן** — לאשר את שינוי ההתנהגות (פסילת גיבויים ישנים שבורים).
- **עלות?** אין.

### 2. ❄️ לא בסשן הזה — מסלול B (Supabase אמיתי, R-23)
