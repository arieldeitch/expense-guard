...
## ADR-0044 · 2026-09-24 · חוזה אחד להזנת משך ומספר עשרוני

**הקשר:** פידבק אריאל משימוש אמיתי: `42:15` ב-Suunto נקרא כ-42.15 דקות; `7.15` במרחק "נשבר" תוך כדי הקלדה; משך ריצה של 40 שניות דרש להקליד `0:40`. השורש היה שלושה באגים נפרדים — parse שמניח דקות למספר חשוף, parser שני ל-pace, ו-input מבוקר שממיר ל-`Number` בכל תו ולכן בולע את הנקודה העשרונית.

**החלטה:** parser מרכזי אחד ב-`src/lib/runs/calc.ts`. `parseDurationInput` מקבל `ss` · `mm:ss` · `hh:mm:ss` בלבד; מספר חשוף הוא **שניות**; כל חלק אחרי הראשון הוא 0..59; כל דבר אחר מחזיר `null` במקום לשמור ערך שגוי. `parsePaceMSS` מאציל לאותה פונקציה. `formatDurationInput` מקנן חזרה (`0:40`, `42:15`, `1:02:03`) ו-`describeDuration` מחזיר הד מילולי ("42 דקות ו-15 שניות"). `parseDecimal` מקבל גם פסיק עשרוני. `src/lib/forms/useNumericText.ts` שומר את הטקסט הגולמי בזמן הקלדה, כך ש-`7.` שורד עד שהוא הופך ל-`7.15`, וה-blur מקנן. `DecimalField` ו-`DurationTextField` (`src/components/inputs/NumericField.tsx`) הם הצורה היחידה שבה טופס Suunto מזין מספרים.

**נימוק:** שני parsers הם שני מקורות אמת ולכן שני באגים. הד מילולי הופך קריאה שגויה לגלויה **לפני** השמירה, במקום להסתמך על שהמשתמש יזהה פורמט.

**השלכה:** `parsePaceMSS("30")` היה 1800 והוא כעת 30 — שינוי התנהגות מכוון, מעוגן ב-`calc.test.ts` וב-`duration-contract.test.ts`. `DurationField` (דקות/שניות בשני שדות) נשאר ב-`RunForm`: הוא חד-משמעי מבנית ואינו זקוק לחוזה הטקסטואלי.

## ADR-0045 · 2026-09-24 · פק״לים הם כמות, לא זמן — שתי שגרות קבועות הניתנות לעריכה

**הקשר:** דיווח פק״לים נבנה סביב משך אימון, בעוד שבפועל אריאל סופר חזרות. דיווח קיים לא היה ניתן לעריכה, ושם האימון ירש את שם התרגיל הראשון.

**החלטה:** `src/lib/home/pakalim.ts` מגדיר שתי שגרות קבועות עם מזהי template דטרמיניסטיים — `tpl_pakal_morning` ו-`tpl_pakal_evening`. `ensurePakalTemplate` הוא idempotent ו**אינו** דורס template קיים. `openPakalSession` מחזיר את הדיווח של היום (מפתח יום Asia/Jerusalem) אם קיים, ורק אחרת יוצר חדש — ולכן עריכה היא עריכה, לא delete+recreate. `pakalLines` הוא pure ונקרא ב-render; סט נוצר רק בעריכה הראשונה. המסך `/home/pakal/$slot` הוא כמות-תחילה: שדה מספרי גדול עם ± בגודל 44px, שינוי סדר, הסרה, הוספה מהבנק, סיכום חזרות ושינוי שם השגרה. שם ה-session הוא snapshot של שם ה-template; `startQuickEntry` כבר אינו מעתיק `name_he` של תרגיל.

**נימוק:** מדד הפק״ל הוא חזרות. שגרה קבועה עם מזהה יציב היא מה שמאפשר "לפתוח שוב ולתקן" בלי לשכפל היסטוריה.

**השלכה:** שורת היסטוריה של פק״ל מציגה את שם השגרה ככותרת והתרגילים כהקשר. אין migration: רשומות קיימות נשארות כפי שהן. מסלול חדש `/home/pakal/$slot` ב-`routeTree.gen.ts`.

## ADR-0046 · 2026-09-24 · בנק תרגילי בית לפי ציוד וקבוצת שריר, עם איורים

**הקשר:** בנק התרגילים הביתי הכיל משקל גוף בלבד, והבחירה הייתה רשימה שטוחה. אריאל ביקש דאמבלים וחבל, ניווט לפי קבוצת שריר, התרגיל הנפוץ ראשון, ואיור שמראה מה עובד ואיך התנועה נראית.

**החלטה:** `src/lib/exercises/homeBank.ts` מגדיר שלוש קטגוריות ציוד (משקל גוף · דאמבלים · חבל) וחמש קבוצות שריר לדאמבלים; קבוצה מוגדרת **בקבוצת קודי שריר** (`["back","lats","traps"]`) ולא בקוד יחיד, כי `single-arm-dumbbell-row` נושא `lats`. לכל קבוצה `firstSlug` — התרגיל הנפוץ, שממוין ראשון ומסומן "הנפוץ ביותר". `HomeExerciseAdder` הוא בורר דו-שלבי עם breadcrumb וחיפוש. `MuscleBadge` הוא צללית קטנה לפי טקסונומיית `BodyRegion` הקיימת; `ExerciseMovementIllustration` מצייר זוג תנוחות START→END מתוך `MOVEMENT_BY_SLUG`. שניהם מקבלים `decorative` ואז הם `role="presentation"` — איור בתוך כפתור אינו חלק מהשם הנגיש שלו.

**נימוק:** ניווט ציוד→שריר→תרגיל הוא שתי הכרעות קצרות במקום סריקת רשימה; איור מזהה תרגיל מהר יותר משם עברי.

**השלכה:** ה-seed קיבל רק `Wide Push Ups` ו-`Dumbbell Floor Press`; ארבעה תרגילים שהוצעו כבר היו קיימים תחת slugs אחרים ולכן הקבוצות מצביעות עליהם במקום לשכפל. `MuscleMap.tsx` הקיים לא שונה.

## ADR-0047 · 2026-09-24 · חוזה בעלות אחד — המודל אינו חד-משתמשי, וה-UX נשאר חד-משתמשי

**הקשר:** נדרשה מוכנות ליותר ממשתמש אחד ברמת הנתונים, **בלי** ליצור משתמש שני, UX של family management או בקרות הורה/ילד, ובלי להמציא user id ל-Ariel ולקבע אותו כקבוע קסם.

**החלטה:** `src/lib/identity/owner.ts` הוא המקום היחיד שבו נקבע ה-owner. `LOCAL_OWNER_ID = "single-user"` — **הערך המאוחסן אינו משתנה**, ולכן אין migration ואף רשומה קיימת אינה זזה. `SYSTEM_OWNER_ID` מסמן קטלוג משותף. `currentOwnerId()` הוא הנקודה היחידה שבה יישב בעתיד `auth.uid()`; `withOwner()` הוא תפר לבדיקות/bootstrap; `visibleToCurrentOwner()` מסנן לפי בעלים ומתייחס לרשומה ישנה ללא `owner_id` כשייכת לבעלים המקומי, כדי ששום דבר לא ייעלם. שמונה מודולי אחסון מייצאים כעת `CURRENT_OWNER_ID` מהמקור הזה.

**נימוק:** הכשל שממנו נמנעים הוא בעלות מפוזרת כמחרוזת קבועה בכל מודול. ריכוז עכשיו הופך חיבור auth עתידי לשינוי בנקודה אחת.

**השלכה:** אין שינוי סכימה, אין שינוי RLS, אין policy חדש, ואין נתוני משתמש שני. `owner.test.ts` מוכיח שהמודל יכול להחזיק שני בעלים — האפליקציה עצמה לעולם אינה קוראת ל-`withOwner` מחוץ לבדיקות.
