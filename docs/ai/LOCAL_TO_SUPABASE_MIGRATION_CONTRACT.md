...
## ✅ החוזה הורץ (2026-07-26) — Fake Supabase rehearsal

עד 2026-07-26 המסמך הזה היה **הנחה שלא נבדקה**. כעת הוא מורץ בכל ריצת בדיקות מול
**ענן מדומה בזיכרון** (`src/lib/migration/`), ללא Supabase, SDK, SQL, רשת, env או עלות.

- **`CLOUD_ENTITIES`** (`src/lib/migration/cloudSchema.ts`) הוא המימוש המכונתי של טבלת המיפוי למטה: 30 ישויות, כל אחת עם מפתח יציב, קשרי הורה, שדה סדר וסוג בעלות.
- **סדר הייבוא מחושב טופולוגית** מהקשרים שבמפה — הוא אינו רשימה ידנית ואינו יכול לסטות ממנה. עמודת "Import order" למטה נשמרת כתיעוד קריא בלבד.
- **מה שהוכח:** הורה לפני ילד · ילד יתום נדחה ואינו נכתב · ייבוא חוזר = no-op מלא · קונפליקט מפורש ללא דריסה · מזהים, קשרים, סדר וחותמות זמן שורדים · אוסף ללא mapping מדווח.
- **מה שעדיין לא הוכח:** RLS, Auth, FK constraints של מנוע אמיתי, טרנזקציות, טיפוסי עמודות, רשת וביצועים. **נדרש מבחן Import אמיתי מול Supabase עם Auth/RLS.**

ראה ADR-0034 (rehearsal), ADR-0035 (בעלות), ADR-0036 (Readiness Gate).

### דיוק לעמודת Ownership

טבלת המיפוי רושמת `auth.uid()` באופן גורף. **המימוש מדייק:** רשומה עם
`is_system === true` (muscle groups ותרגילי seed) מקבלת `user_id: null` ואינה
בבעלות משתמש; תרגיל מותאם (`is_system: false`) כן מקבל בעלות. ראה ADR-0035.

### הפרדה מחייבת — שלוש שכבות

| שכבה | מקור | תפקיד |
|---|---|---|
| record identity | ה-`id` שבקובץ | שורד ללא שינוי; PK גם בענן |
| cloud ownership | **אך ורק** `authenticatedUserId` | סמכות ההרשאה |
| source metadata | `owner_id`/`user_id` שבקובץ | `source_metadata.source_owner_id`, תיעוד בלבד |

**שדות סוד** (`token`, `secret`, `api_key`, `service_role`, `password`, `credential`)
לעולם אינם עוברים ל-payload, גם אם הופיעו בקובץ.
...
- ~~**החוזה הזה מעולם לא הורץ.**~~ ✅ **בוצע 2026-07-26** — ראה סעיף ה-rehearsal בראש המסמך.
- **לא בוצע ייבוא מול Supabase אמיתי.** הענף שנבדק הוא ענן מדומה בזיכרון. **זהו הפער העיקרי שנותר.**
- **אין הגדרת RLS policies** ולא נבדק שהן אוכפות את הבעלות שה-pipeline מקצה.
- **אין FK constraints מוגדרים.** סדר הייבוא הוכח מול הגרף שלנו, לא מול מנוע שאוכף אותו.
- **לא נבדקו טיפוסי עמודות** (numeric precision, JSONB, timestamptz) ולא התנהגות טרנזקציות/rollback ברמת ה-DB.
- `owner_id` המקומי נדרס בייבוא ונשמר כ-`source_metadata` בלבד — יש לוודא שאין הסתמכות עליו בקוד לפני המעבר.

### פער מוכח באימות המקומי (P1, טרם תוקן)
...
לכן הכלל **אינו יורה לעולם**. בנוסף, כל כלל מדלג כשאין אף רשומת הורה
(`if (parents.size === 0) continue`), כך ש"כל ההורים נמחקו" אינו נתפס.

**השפעה:** נמוכה בפועל — ה-import pipeline בונה גרף תלויות משלו מ-`CLOUD_ENTITIES`,
תופס את המקרה, ודוחה את הרשומה. יש בדיקה שמתעדת בדיוק את זה
(`importPipeline.test.ts`). **התיקון פתוח** ב-`open-tasks.md`.
