...
import type { MigrationResult } from "@/lib/storage/migrations";
 ...
 */
export function pickStorageNotice(
  worstStatus: StorageWriteStatus,
  migration: MigrationResult | null = null,
): StorageNotice | null {
  // כשל מיגרציה או גרסה עתידית קודמים לכל השאר: הם מתארים נתונים שאי אפשר
  // לעבוד איתם בכלל, לא כתיבה בודדת שנכשלה.
  if (
    migration &&
    (migration.status === "migration_failed" || migration.status === "future_version_blocked")
  ) {
    return {
      role: "alert",
      title: "בעיה בנתונים המקומיים",
      message: migration.message ?? FAILED_MESSAGE,
    };
  }
  ...
  if (migration && migration.status === "storage_unavailable") {
    return {
      role: "status",
      title: "אזהרת שמירה",
      message: migration.message ?? MEMORY_ONLY_MESSAGE,
    };
  }
  return null;
}
